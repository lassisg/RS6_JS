...
let taskValidationMessage = document.querySelector(
    ".invalid-feedback.invalid-task"
);
let taskRepeatedMessage = document.querySelector(
    ".invalid-feedback.repeated-task"
);
// Validar data (não pode ser no passado)
let taskDateValidationMessage = document.querySelector(
...
// DONE: Mostrar a data no formato dd/mm/aaaa
function getFormattedDate(inputDate) {
    let formattedDate = new Date(inputDate).toLocaleDateString();
    return formattedDate;
}
function getDefaultFormatDate(inputDate) {
    let defaultFormattedDate = inputDate.split("/").reverse().join("-");
...
function addTask(task) {
    ...
    let data = getFormattedDate(task.data);
    ...
    // DONE: Hide buttons if completed
}
...
formTarefa.addEventListener("submit", function (event) {
    ...
    let isRepeatedTask = tasks.find(
        (tarefa) => tarefa.tarefa.toUpperCase() === taskText.toUpperCase()
    );
        ...
        if (!isRepeatedTask || isEdit) {
            if (isEdit) {
                let currentTask = {
                    ...
                    data: campoDataLimite.value,
                };
                let tarefaAtual = document.querySelector(".edit");
                let textoAtual = tarefaAtual.querySelector(
                    ".tarefa-texto > span"
                ).textContent;

                let taskIndex = tasks.findIndex(
                    (tarefa) =>
                        tarefa.tarefa.toUpperCase() === textoAtual.toUpperCase()
                );

                tarefaAtual.querySelector(".tarefa-texto > span").textContent =
                    currentTask.tarefa;
                tarefaAtual.querySelector(".tarefa-data > span").textContent =
                    getFormattedDate(currentTask.data);
...
function toggleTaskStatus(currentElement) {
    ...
    let currentTaskText = currentElement.querySelector(
        ".tarefa-texto > span"
    ).textContent;

    let currentTask = tasks.find(
        (tarefa) =>
            tarefa.tarefa.toUpperCase() === currentTaskText.toUpperCase()
    );
...
taskTable.addEventListener("click", function (event) {
    ...
    if (isRemoveButton) {
        ...
        tasks.forEach((item, pos) => {
            let itemDate = getDefaultFormatDate(tarefaProps[1].textContent);
            if (
                item.tarefa === tarefaProps[0].textContent &&
                item.data === itemDate
            ) {
                tasks.splice(pos, 1);
            }
    ...
    if (isEditButton) {
        ...
        campoTarefa.value = tarefa.querySelector(
            ".tarefa-texto > span"
        ).textContent;
        let formattedDate = tarefa.querySelector(
            ".tarefa-data > span"
        ).textContent;
        campoDataLimite.value = formattedDate.split("/").reverse().join("-");
